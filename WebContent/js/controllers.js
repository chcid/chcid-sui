...
angular.module('speechApp.controllers', []).controller(
		'recordController',
		[
				'$scope',
				'dataFactory',
				...
				function($scope, dataFactory, $routeParams) {
					...
					function deleteRecord(id) {
						dataFactory.deleteRecord(id, TableName).success(
								function(result) {
									getAll();
								}).error(
								function(error) {
					...
					$scope.submitUpdate = function(record) {
						if (record[IdColName]) {
							updateRecord(record);
							console.log(TableName + " " + record[IdColName]
									+ " updated", record);
							$scope.recordToUpdate = {};
						} else {
							insertRecord(record);
					...
					var updateRecord = function(record) {
						dataFactory.updateRecord(record, TableName).success(
								function() {
									$scope.status = 'Updated ' + TableName
					...
					var insertRecord = function(record) {
						dataFactory.insertRecord(record, TableName).success(
								function() {
									$scope.status = 'Inserted ' + TableName
											+ '! Refreshing ' + TableName
											+ ' list.';
									getAll();
								}).error(
